...
public class ComplexFilterSplitter extends PostPreProcessFilterSplittingVisitor {
    ...
    protected void visitBinarySpatialOperator(BinarySpatialOperator filter) {
        ...
        if (preStack.size() == i + 1) {
            if (nestedAttributes.size() == 1) {
                ...
                // encoding a spatial comparison between multiple nested attributes is not supported)
                Object o = preStack.pop();
                postStack.push(o);
            }
        }
        ...
        if (AppSchemaDataAccessConfigurator.shouldEncodeNestedFilters()) {
            ...
            if (fcAttrs.size() == 0 && !nestedAttrExtractor.conditionalMappingWasFound()
                ...
                throw new IllegalArgumentException(
                        String.format("Attribute \"%s\" not found in type \"%s\"", expression,
        ...
        if (nestedAttributes != null) {
            for (FeatureChainedAttributeDescriptor descr : nestedAttributes) {
                ...
                if (Types.isSimpleContentType(ownerType.getTargetFeature().getType())) {
                    // if chaining a simple content type, XPath expression may point directly to the type,
                    // and not to one of its attributes: if this is the case, the XPath expression should
                    // be tested against the container type, i.e. the previous link in the chain
                    String ownerTypeName = Types.toPrefixedName(
                            ownerType.getTargetFeature().getName(), ownerType.getNamespaces());
                    boolean chainingSimpleType = ownerTypeName
                            .equals(descr.getAttributePath().toString());
                    ...
                    if (chainingSimpleType && lastLink.previous() != null) {
                        ...
                        StepList prevXPathSteps = lastLink.previous().getNestedFeatureAttribute()
                                .getTargetXPath();
                        ...
                        LOGGER.warning(String.format(
                                ...
                                descr.getAttributePath(), ownerType.getTargetFeature().getName()));
                    }
                }
    ...
    private void checkPropetyExistenceAndType(FeatureTypeMapping mapping, String xpath,
            Class<?> expectedType) {
        ...
        if (accessor != null) {
            ...
            try {
                ...
                throw new IllegalArgumentException(
                        String.format("Attribute \"%s\" not found in type \"%s\"", xpath,
                                mappings.getTargetFeature().getName().toString()), e);
            }
